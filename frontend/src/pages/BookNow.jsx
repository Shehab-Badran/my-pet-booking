...
export default function BookNow({
  ...
  const handleConfirmBooking = async (e) => {
    ...
    const phoneToSubmit = (currentUser ? currentUser.phone : customerPhone).trim();
    ...
    if (!phoneToSubmit || phoneToSubmit.length < 10) {
    ...
    try {
      ...
      if (!token) {
        ...
        if (authRes.ok && authData.access_token) {
          ...
          throw new Error(authData.detail || 'Authentication failed. Please log in first.');
        }
      }
      ...
      if (!bookingRes.ok) {
        throw new Error(bookingData.detail || 'Failed to confirm booking.');
      }
